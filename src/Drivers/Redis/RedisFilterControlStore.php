...
final readonly class RedisFilterControlStore implements ActiveGenerationSnapshotReader, FilterControlStore
{
    ...
    public function readActive(FilterName $name): ?ActiveGenerationSnapshot
    {
        ...
        $activeVersionToken = $response[2];
        $decoded = $this->codec->decode($name, [
            'format',
            'control-v1',
            'revision',
            $response[1],
            ...
            'active_version',
            $activeVersionToken,
            sprintf('g:%s:lifecycle', $activeVersionToken),
            $response[3],
            sprintf('g:%s:health', $activeVersionToken),
            $response[4],
        ]);
        $activeVersion = $decoded->activeVersion();
        $generation = $decoded->generations()[0] ?? null;

        if ($activeVersion === null || $generation === null) {
            throw new FilterControlStateCorrupt(
                'Redis active-generation control snapshot is incomplete.',
            );
        }
